...
function installNativeQueueParser(){
 ...
 if(!bridge?.execute||typeof original!=='function'||original.__cybertrmxQueue535)return false;
 const wrapped=async raw=>{const cmd=String(raw||'').trim().split(/\s+/)[0]?.toLowerCase();if(cmd==='job'||cmd==='lookup')return bridge.execute(raw);return original(raw)};
 wrapped.__cybertrmxQueue535=true;wrapped.__cybertrmxOriginal=original;window.execute=wrapped;
 try{window.CYBERTRMX_PRODUCTION_EXECUTE=wrapped;(0,eval)('execute = window.CYBERTRMX_PRODUCTION_EXECUTE')}catch{}
 window.dispatchEvent(new CustomEvent('cybertrmx:queue-parser-ready',{detail:{version:V}}));return true;
}
async function boot(){
 ...
 try{
  ...
  await load('./device-transport-v535.js','cybertrmx-device-transport-v535');
  ...
  await load('./r3-terminal-bridge.js','cybertrmx-terminal-v53');await load('./queue-cleanup-v532.js','cybertrmx-queue-cleanup-v533');
