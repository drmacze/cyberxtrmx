...
function installNativeQueueParser(){const bridge=window.CYBERTRMX_R3_TERMINAL_BRIDGE,original=window.execute;if(!bridge?.execute||typeof original!=='function'||original.__cybertrmxQueue536)return false;const wrapped=async raw=>{const cmd=String(raw||'').trim().split(/\s+/)[0]?.toLowerCase();if(cmd==='job'||cmd==='lookup')return bridge.execute(raw);return original(raw)};wrapped.__cybertrmxQueue536=true;wrapped.__cybertrmxOriginal=original;window.execute=wrapped;try{window.CYBERTRMX_PRODUCTION_EXECUTE=wrapped;(0,eval)('execute = window.CYBERTRMX_PRODUCTION_EXECUTE')}catch{}window.dispatchEvent(new CustomEvent('cybertrmx:queue-parser-ready',{detail:{version:V}}));return true}
async function boot(){
 addStyle('./cloud-core.css','cybertrmx-cloud-style');addStyle('./security-v52.css','cybertrmx-security-style');addStyle('./guard-v528.css','cybertrmx-guard-style');addStyle('./jobs-r2.css','cybertrmx-jobs-v53-style');if(STABILITY_536)addStyle('./stability-v536.css','cybertrmx-stability-v536-style');
 ...
 try{
  ...
  if(STABILITY_536)await load('./stability-transport-v536.js','cybertrmx-stability-transport-v536');
