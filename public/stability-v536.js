...
function installTerminal(){if(terminalInstalled||typeof window.execute!=='function')return false;const original=window.execute;const wrapped=async raw=>{const value=String(raw||'').trim().toLowerCase();if(value==='health'){terminalLine('running backend health checks…','dim');await runChecks();report().split('\n').forEach(line=>terminalLine(line,line.includes(' ERROR ')?'warn':'block'));return}if(value==='health copy'){await copyReport();terminalLine('health report copied','ok');return}return original(raw)};wrapped.__cybertrmxHealth536=true;wrapped.__cybertrmxOriginal=original;window.execute=wrapped;try{window.CYBERTRMX_HEALTH_EXECUTE=wrapped;(0,eval)('execute = window.CYBERTRMX_HEALTH_EXECUTE')}catch{}terminalInstalled=true;return true}
window.addEventListener('cybertrmx:diagnostics',event=>{state.lastDiagnostic=event.detail||null});
function boot(){mount();observeQueue();installTerminal();let attempts=0;const timer=setInterval(()=>{attempts++;mount();observeQueue();installTerminal();if((mount()&&observeQueue()&&terminalInstalled)||attempts>=120)clearInterval(timer)},100);window.CYBERTRMX_STABILITY_536={version:VERSION,runChecks,report,copyReport,state}}
