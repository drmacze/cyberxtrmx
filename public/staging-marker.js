...
function mount(){
  ...
  badge.textContent='STAGING / 5.3.0-r3 / LOADING';
  ...
  const timer=setInterval(()=>{
    attempts++;
    if(window.CYBERTRMX_JOBS_R3?.enabled){badge.textContent='STAGING / 5.3.0-r3 / READY';clearInterval(timer);return}
    if(attempts>=80){badge.textContent='STAGING / 5.3.0-r3 / LOAD ERROR';clearInterval(timer)}
  },100);
}
