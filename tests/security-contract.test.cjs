...
test('PWA caches every active R3 security, guard, and queue asset',()=>{
  ...
  for(const asset of ['security-utils.js','security-r3.js','security-v52.css','command-hints-v52.js','tracker-engine.js','tracker-engine.css','recovery-527.js','guard-v528.js','guard-v528.css','recover.html','jobs-r3.js','r3-terminal-bridge.js','jobs-r2.css'])assert.match(sw,new RegExp(asset.replaceAll('.','\\.')));
  assert.match(sw,/cybertrmx-v46-r3/);
...
test('R3 jobs and security install before Operations core',()=>{
  ...
  assert.ok(jobs<core,'jobs-r3 must register before cloud-core');
  assert.doesNotMatch(bootstrap,/jobs-r2\.js|security-v52\.js|transport-v523|patch-click-v525/);
});

test('R3 terminal bridge owns job and lookup commands before the legacy parser',()=>{
  const source=read('public/r3-terminal-bridge.js');
  ...
  assert.match(source,/current\.call/);
  assert.doesNotMatch(source,/command not found/);
});
