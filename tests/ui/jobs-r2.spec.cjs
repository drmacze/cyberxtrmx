...
window.supabase={createClient(){
 ...
 const auth={
  ...
  mfa:{
   ...
   enroll:async()=>({data:null,error:{message:'Smoke test MFA disabled'}}),
    ...
    if(url.pathname.includes('/functions/v1/')){
      ...
      if(url.pathname.endsWith('/cybertrmx-jobs')){
        ...
        if(body.action==='job_status'){
          job={...job,status:'completed',progress:100,attempt_count:1,result:{records:{A:[{data:'203.0.113.10'}]}}};
          return route.fulfill({status:200,headers:{...cors(),'x-cybertrmx-backend':'jobs-smoke'},body:JSON.stringify({ok:true,request_id:'status-request',backend_version:'jobs-smoke',job,events:[{progress:0,stage:'QUEUED',message:'Accepted'},{progress:100,stage:'COMPLETED',message:'Evidence sealed'}],poll_after_ms:10000})});
        }
        if(body.action==='queue_status'){
          const jobs=job?[job]:[];
          const counts=job?[{status:job.status,count:1}]:[];
...
test('staging queue completes a lookup without breaking Operations',async({page})=>{
  const errors=[];page.on('pageerror',error=>errors.push(error.message));
  await installRoutes(page);
  await page.goto('/?jobs_r2=1',{waitUntil:'domcontentloaded'});
  ...
  await expect(page.locator('#ops-r2-queue-card')).toBeVisible();
  ...
  await page.locator('#ops-lookup-form').evaluate(form=>form.requestSubmit());
  await expect(page.locator('#ops-job-label')).toHaveText('COMPLETED',{timeout:7000});
  await expect(page.locator('#ops-job-progress')).toHaveText('100%');
  await expect(page.locator('#ops-runtime-state')).toHaveText('ONLINE');
  await expect(page.locator('#ops-r2-queue-card')).toBeVisible();
