...
test('the stable shell boots without a blank screen',async({page})=>{
  ...
  expect(state.text).toBeGreaterThan(250);
...
test('iPhone controls remain inside the viewport and accept taps',async({page},testInfo)=>{
  ...
  const errors=await openStableShell(page);
  const trigger=page.locator('.floating-trigger');
  await expect(trigger).toBeVisible();
  await trigger.click();
  await expect(page.locator('.floating-nav')).toHaveClass(/open/);
  ...
  await page.locator('#rail .nav-item[data-tab="terminal"]').click();
